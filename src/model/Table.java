...
public class Table {
    	...
    	for (Record record : records) {
			if(record.getValue(key).equals(value)){
				if(record.getValue(primaryKey)!=null){
					...
					for (Table table : refrences) {
						table.removeByForeignKey(table, value);
					}
				}
     ...
     * @throws FKConstrain 
     */
    private void updateByForeignKey(Table t,String value) throws FKConstrain{
    	if (!onUpdate)
    		throw new FKConstrain();
    ...
    /**
     * groups the table according to given variables
     * @param vars	columns name for groupBy
     * @return	returns a 2D array of records and the records like them
     */
    public ArrayList<Record[]> groupBy(String[] vars){
    	...
    	while(records.size() > 0){
    		...
    		for (int i = 0; i < records.size();) { // each time it iterates over the linkeList and finds the same records
				boolean toAdd = true;
    			for (int j = 0; j < vars.length; j++) { // checks if all the given columns are the same or not
					if (! record2.getValue(vars[j]).equals(records.get(i).getValue(vars[j]))){
						toAdd = false;
						break;
					}
				}
    			if(toAdd){ // if the record is the same it adds it to the ArrayList 
    				sameRecords.add(records.get(i));
    				records.remove(records.get(i));
    			}else{
    				i++;
    			}
			}
    		answer.add(sameRecords.toArray(new Record[sameRecords.size()]));
    	}	
    	return answer;
    }
